...
for index in range(1, 2):
    ...
    for score in scores:
        # f = open('train_movie_review%d.txt' % score_index, 'w', encoding='utf-8')
        str_score = str(score)
    ...
    for link in links:
        # f = open('train_movie_review%d.txt' % review_index, 'a', encoding='utf-8')
        str_link = str(link)
        movie_title = link.find('a', {'class': 'movie'}).text
